...
public class AvgRespFileSearch implements LeafNodeInterface {
    ...
    AvgRespFileSearch(String portNo, String dirName, String superpeer, String peerID, String cachedDirName){
        this.portNo = portNo;
...
//							spInter.query(msgId, timeTL, fileName, peerID, portNo);
                        ...
                        if(!finalRes.isEmpty()){
                            ...
                            for (ArrayList<String> as1 : finalRes){
                                ...
                                int val = dtf.parse(as1.get(8)).compareTo(dtf.parse(lmTime));
                                //System.out.println("Value in percentage method is" + val +" VALUES ARE : "+ as1.get(8) + lmTime);
                                if(!(as1.get(7).equalsIgnoreCase(refValid))){
                                    invalid_count++;
                                }
                            }
                            double percentage = (invalid_count / (double)finalRes.size()) * 100.0;
                            System.out.println("Percentage of Invalid query results : "+ invalid_count + " and  % "+percentage);
